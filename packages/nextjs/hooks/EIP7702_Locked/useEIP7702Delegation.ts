...
export const useEIP7702Delegation = () => {
  ...
  }, [address, targetNetwork, walletClient, publicClient]);
  ...
  const checkDelegation = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      // In a real implementation, check actual delegation status with MetaMask SDK
      // For now, we'll simulate checking delegation status
      console.log("Checking delegation status for address:", address);

      // Simulate API call to check delegation status
      await new Promise(resolve => setTimeout(resolve, 500));

      // Mock delegation status - in reality, this would be checked via MetaMask SDK
      const isActive = Math.random() > 0.5; // Random for demo purposes

      if (isActive) {
        console.log("Delegation is active:", true);
        setIsDelegationEnabled(true);
      } else {
        console.log("Delegation is not active or has expired.");
        setIsDelegationEnabled(false);
      }
    } catch (error) {
  ...
  // Enable delegation through MetaMask SDK
  const enableDelegation = useCallback(async () => {
    if (!address) {
    ...
    try {
      setIsLoading(true);
      // In a real implementation, this would call MetaMask SDK to enable delegation
      // For now, we'll simulate the process
      console.log("Enabling delegation for address:", address);

      // Simulate API call to enable delegation
      await new Promise(resolve => setTimeout(resolve, 1000));

      // Mock successful delegation
      const result = { success: true, message: "Delegation enabled" };
      console.log("Delegation enabled successfully:", result);
      await checkDelegation(); // Re-check status after enabling
    } catch (error) {
      console.error("Failed to enable delegation:", error);
      notification.error("Failed to enable delegation");
    } finally {
      setIsLoading(false);
    }
  }, [address, checkDelegation]);

  // Disable delegation through MetaMask SDK
  const disableDelegation = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      // In a real implementation, this would call MetaMask SDK to disable delegation
      // For now, we'll simulate the process
      console.log("Disabling delegation for address:", address);

      // Simulate API call to disable delegation
      await new Promise(resolve => setTimeout(resolve, 1000));

      // Mock successful delegation disable
      console.log("Delegation disabled successfully.");
      await checkDelegation(); // Re-check status after disabling
    } catch (error) {
      console.error("Failed to disable delegation:", error);
      notification.error("Failed to disable delegation");
    } finally {
      setIsLoading(false);
    }
  }, [address, checkDelegation]);
    ...
    async (action: string, params?: any[]) => {
      ...
      try {
        ...
        if (params && params.length > 0) {
          console.debug("Executing action with params:", params);
        }
